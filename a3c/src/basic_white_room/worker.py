...
#                          [-4.5, -20, 1.0, 0, 1.57, 0]]
        aruco_pos_list = [[-4.5, 0, 1.0, 0, 1.57, 0],
                          [4.5, 0, 1.0, 0, 1.57, 3.14],
                          [0, -4.5, 1.0, 0, 1.57, 1.57],
                          [0, 4.5, 1.0, 0, 1.57, -1.57]]
...
#        rospy.logdebug("Laser range punishment: " + str(laser_cost))
        ...
        if self.aruco_msg != None:
            ...
            if np.isnan(aruco_cost):
                ...
                rospy.logdebug("Aruco distance reward: " + str(aruco_cost))
                drone_orient = np.array([self.aruco_msg.angular.x, self.aruco_msg.angular.z]) 
                des_orient = np.array([-self.aruco_msg.linear.x, -self.aruco_msg.linear.z])
#                print ("drone orientation norm: " + str(np.linalg.norm(drone_orient)))
#                print ("desired orientation norm: " + str(np.linalg.norm(des_orient)))
                if np.linalg.norm(drone_orient) != 0 and np.linalg.norm(des_orient) != 0:
                    orient_error = np.arccos(np.inner(drone_orient, des_orient) / (np.linalg.norm(drone_orient) * np.linalg.norm(des_orient))) 
                else:
                    orient_error = np.pi
                aruco_angle_cost = -abs(orient_error / np.pi)            
#                print ("Orientation error: " + str(orient_error) +
#                       ". Aruco angle cost: " + str(aruco_angle_cost))
...
#            laser_cost = 0
        ...
        step_reward = (10 * collision_cost) + \
                      aruco_cost + aruco_angle_cost#+ alt_cost  + \
                      #(0.5 * time_cost) + \
                      #+ laser_cost + angular_cost
        rospy.logdebug("Step reward: " + str(step_reward))
...
#                                  "New angle: " + str(angle))
                    ...
                    # reset the actions
                    # CHECK HEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
#                    delta_t = 0.01
#                    wait_time = 2.0
